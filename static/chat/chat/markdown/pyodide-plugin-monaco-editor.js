...
(function() {
    ...
    function createPythonEditor(editorId, initialCode) {
        ...
        // Для editorBody используем визуальное скрытие вместо display: none
        const editorBodyStyle = isCollapsed ? 'height: 0px; overflow: hidden; visibility: hidden;' : '';
        ...
        return `
            <div class="pyodide-editor-container mb-4" id="${editorId}" data-python-content="${btoa(encodeURIComponent(initialCode))}" data-first-render="true">
                ...
                <div class="pyodide-editor" style="display: ${pluginState.pyodideReady ? 'block' : 'none'};">
                    <div class="card">
                        ...
                        <!-- Редактор кода -->
                        <div class="card-body p-0" style="${editorBodyStyle}">
                            <div class="monaco-editor-container" id="${editorId}-monaco-container" style="height: 300px; min-height: 150px; max-height: 800px; border: none;"></div>
                        </div>
    ...
    async function loadLibrariesInBackground() {
        try {
            ...
            try {
                await pluginState.pyodideInstance.loadPackage('/static/libs/pyodide/certifi-2025.7.14-py3-none-any.whl');
            ...
            try {
                await pluginState.pyodideInstance.loadPackage('/static/libs/pyodide/charset_normalizer-3.4.2-py3-none-any.whl');
            ...
            try {
                await pluginState.pyodideInstance.loadPackage('/static/libs/pyodide/idna-3.10-py3-none-any.whl');
            ...
            try {
                await pluginState.pyodideInstance.loadPackage('/static/libs/pyodide/urllib3-2.5.0-py3-none-any.whl');
            ...
            try {
                await pluginState.pyodideInstance.loadPackage('/static/libs/pyodide/requests-2.32.4-py3-none-any.whl');
...
`);
     ...
     */
    async function processMagicCommandsForEditor(editorId) {
        ...
        // ВАЖНО: Сначала обрабатываем команду #%load для загрузки шаблона (СИНХРОННО)
        if (state.hasLoadCommand) {
            const monacoContainer = document.getElementById(editorId + '-monaco-container');
            if (monacoContainer) {
                let currentCode = '';
                if (monacoContainer._monacoEditor) {
                    currentCode = monacoContainer._monacoEditor.getValue();
                } else if (monacoContainer.getValue) {
                    currentCode = monacoContainer.getValue();
                }
                
                try {
                    // ЖДЕМ завершения загрузки шаблона
                    const processedCode = await parseLoadCommand(currentCode);
                    if (processedCode !== currentCode) {
                        // Обновляем содержимое редактора с загруженным шаблоном
                        if (monacoContainer._monacoEditor) {
                            monacoContainer._monacoEditor.setValue(processedCode);
                        } else if (monacoContainer.setValue) {
                            monacoContainer.setValue(processedCode);
                        }
                        
                        // Обновляем редактор для отображения изменений
                        if (monacoContainer._updateEditor) {
                            monacoContainer._updateEditor();
                        }
                        ...
                        if (newMagicCommands.collapse && !state.collapseApplied) {
                            ...
                            if (collapseIcon && collapseBtn && editorCard) {
        ...
        // Обрабатываем команду %autorun (collapse уже была применена в HTML или выше)
        ...
        // Помечаем, что первый рендер обработан (ТОЛЬКО после завершения всех асинхронных операций)
        state.isFirstRender = false;
        container.setAttribute('data-first-render', 'false');
        
        // ВАЖНО: Добавляем в глобальное отслеживание обработанных команд (ТОЛЬКО после завершения)
     ...
     */
    async function processMagicCommandsOnFirstRender() {
        // Проходим по всем редакторам с магическими командами
        for (const [editorId, state] of magicCommandsState.entries()) {
            await processMagicCommandsForEditor(editorId);
        }
    }

    // Переменная для предотвращения избыточных вызовов переинициализации
    ...
    function reinitializeEditorsOnModeChange() {
        ...
        // Устанавливаем новый таймаут с дебаунсингом
        reinitializationTimeout = setTimeout(() => {
            ...
            containers.forEach(container => {
                ...
                // Проверяем, что контейнер Monaco Editor существует
                if (monacoContainer) {
                    // ДОПОЛНИТЕЛЬНАЯ проверка: уже инициализирован ли редактор
                    if (monacoContainer._monacoEditor || monacoContainer.dataset.initializing === 'true') {
                        log('debug', `Редактор ${editorId} уже инициализирован или инициализируется, пропускаем`);
                        return;
                    }
                    ...
                    if (isVisible) {
                        ...
                        if (pluginState.librariesReady) {
                            ...
                            processMagicCommandsForEditor(editorId).catch(error => {
                                log('error', `Ошибка обработки магических команд для ${editorId}:`, error);
                            });
                        }
                    ...
                    log('warn', `Контейнер Monaco Editor не найден для редактора ${editorId}`);
                }
            });
    ...
    // Глобальная переменная для отслеживания инициализации Monaco Editor
    let monacoInitialized = false;
    let monacoInitPromise = null;

    /**
     * Глобальная инициализация Monaco Editor (выполняется один раз)
     */
    async function initializeMonacoGlobally() {
        ...
        monacoInitPromise = new Promise((resolve, reject) => {
            // Проверяем, что Monaco Editor loader загружен
            if (!window.require) {
                reject(new Error('Monaco Editor loader не найден'));
                return;
            }
            ...
            if (window.monaco && window.monaco.editor) {
                ...
                log('debug', 'Monaco Editor уже был загружен');
                resolve();
                return;
            }

            // Настраиваем окружение Monaco Editor для версии 0.52.2
            if (!window.MonacoEnvironment) {
                window.MonacoEnvironment = {
                    getWorkerUrl: function (workerId, label) {
                        // Для Monaco Editor 0.52.2 используем прямые пути к воркерам
                        if (label === 'json') {
                            return '/static/libs/monaco-editor/min/vs/language/json/jsonWorker.js';
                        }
                        if (label === 'css' || label === 'scss' || label === 'less') {
                            return '/static/libs/monaco-editor/min/vs/language/css/cssWorker.js';
                        }
                        if (label === 'html' || label === 'handlebars' || label === 'razor') {
                            return '/static/libs/monaco-editor/min/vs/language/html/htmlWorker.js';
                        }
                        if (label === 'typescript' || label === 'javascript') {
                            return '/static/libs/monaco-editor/min/vs/language/typescript/tsWorker.js';
                        }
                        // Для Python, YAML, Markdown и других языков используем пустой воркер
                        // чтобы избежать ошибок "Cannot read properties of undefined"
                        return 'data:text/javascript;charset=utf-8,' + encodeURIComponent(`
                            // Пустой воркер для языков без специальной поддержки
                            self.addEventListener('message', function(e) {
                                // Просто отвечаем пустым результатом
            ...
            // Настраиваем пути для Monaco Editor только если еще не настроены
            try {
                window.require.config({
                    paths: {
                        'vs': '/static/libs/monaco-editor/min/vs'
                    }
                });
            } catch (configError) {
                log('warn', 'Ошибка настройки путей Monaco Editor (возможно, уже настроены):', configError);
            }
            ...
            window.require(['vs/editor/editor.main'], function() {
                try {
                    monacoInitialized = true;
                    log('debug', 'Monaco Editor глобально инициализирован');
                    resolve();
                } catch (error) {
                    log('error', 'Ошибка глобальной инициализации Monaco Editor:', error);
                    reject(error);
                }
            }, function(error) {
                log('error', 'Ошибка загрузки Monaco Editor:', error);
                reject(error);
            });
        });
    ...
    /**
     * Создание экземпляра Monaco Editor для конкретного редактора
     */
    async function createMonacoEditorInstance(editorId, initialCode) {
        // Убеждаемся, что Monaco Editor глобально инициализирован
        await initializeMonacoGlobally();

        const container = document.getElementById(editorId + '-monaco-container');
        if (!container) {
            throw new Error(`Контейнер Monaco Editor не найден для ${editorId}`);
        }

        // Создаем экземпляр Monaco Editor
        ...
        editor.addCommand(window.monaco.KeyMod.CtrlCmd | window.monaco.KeyCode.Enter, function() {
            runCode(editorId);
        });
        ...
        // Обновляем статистику при изменении позиции курсора
        editor.onDidChangeCursorPosition(() => {
            const code = editor.getValue();
            updateEditorStats(editorId, code);
        });

        log('debug', `Monaco Editor создан для ${editorId}`);
        return editor;
    }
    ...
    async function setupEditor(editorId) {
        ...
        if (!container) {
            log('error', `Контейнер Monaco Editor не найден для ${editorId}`);
            return;
        }

        // СТРОГАЯ проверка: не инициализирован ли уже редактор
        if (container._monacoEditor) {
            log('debug', `Monaco Editor уже инициализирован для ${editorId}, пропускаем`);
            return container._monacoEditor;
        }

        // Дополнительная проверка: есть ли уже Monaco Editor в DOM
        const existingEditor = container.querySelector('.monaco-editor');
        if (existingEditor) {
            log('debug', `Monaco Editor DOM уже существует для ${editorId}, пропускаем создание`);
            return;
        }

        // Проверяем, не находится ли контейнер в процессе инициализации
        if (container.dataset.initializing === 'true') {
            log('debug', `Monaco Editor для ${editorId} уже инициализируется, пропускаем`);
            return;
        }

        // Помечаем контейнер как инициализирующийся
        container.dataset.initializing = 'true';

        try {
            // Получаем начальный код из атрибута контейнера (декодируем base64)
            const editorContainer = document.getElementById(editorId);
            let initialCode = '';
            if (editorContainer) {
                const encodedContent = editorContainer.getAttribute('data-python-content') || '';
                if (encodedContent) {
                    try {
                        initialCode = decodeURIComponent(atob(encodedContent));
                    } catch (error) {
                        log('warn', `Ошибка декодирования содержимого для ${editorId}:`, error);
                        // Fallback на старый способ для совместимости
                        initialCode = encodedContent;
                    }
                }
            }

            // Создаем экземпляр Monaco Editor
            const editor = await createMonacoEditorInstance(editorId, initialCode);
            ...
            // Добавляем методы для совместимости с магическими командами
            container._updateEditor = () => {
                // Для Monaco Editor обновление происходит автоматически
                const code = editor.getValue();
                updateEditorStats(editorId, code);
            };

            // Добавляем методы для получения и установки значения (для совместимости)
            container.getValue = () => editor.getValue();
            container.setValue = (value) => editor.setValue(value);

            log('debug', `Monaco Editor настроен для ${editorId}`);
            
            // Убираем флаг инициализации после успешного создания
            delete container.dataset.initializing;
            return editor;

        } catch (error) {
            log('error', `Ошибка настройки Monaco Editor для ${editorId}:`, error);
            
            // Убираем флаг инициализации при ошибке
            delete container.dataset.initializing;
            
            // В случае ошибки создаем fallback textarea
            createFallbackEditor(editorId, container);
        }
    }

    /**
     * Создание fallback редактора на основе textarea (на случай ошибки Monaco Editor)
     */
    function createFallbackEditor(editorId, container) {
        log('warn', `Создание fallback редактора для ${editorId}`);
        ...
        if (editorContainer) {
            ...
            if (encodedContent) {
                try {
                    ...
                    log('warn', `Ошибка декодирования содержимого для fallback редактора ${editorId}:`, error);
                    // Fallback на старый способ для совместимости
                    initialCode = encodedContent;
                }
            }
        ...
        container.innerHTML = `
            <textarea
                ...
                style="width: 100%; height: 300px; min-height: 150px; max-height: 800px;
                       font-family: 'Courier New', monospace; font-size: 14px; line-height: 1.5;
                       padding: 15px; border: 1px solid var(--bs-border-color);
                       background: var(--bs-body-bg); color: var(--bs-body-color);
                       resize: vertical; tab-size: 4;"
                placeholder="# Введите Python код здесь..."
            >${escapeHtmlForTextarea(initialCode)}</textarea>
        `;
        ...
        if (textarea) {
            ...
            container.setValue = (value) => { textarea.value = value; };
            container._updateEditor = () => updateEditorStats(editorId, textarea.value);
            
            // Обработчики событий
            textarea.addEventListener('input', () => updateEditorStats(editorId, textarea.value));
    ...
    function updateEditorStats(editorId, code) {
        ...
        if (cursorInfo && charCount && lineCount) {
            const container = document.getElementById(editorId + '-monaco-container');
            let currentLine = 1;
            let currentColumn = 1;
            
            if (container && container._monacoEditor) {
                // Для Monaco Editor получаем позицию курсора
                const position = container._monacoEditor.getPosition();
                if (position) {
                ...
                const textarea = document.getElementById(editorId + '-fallback-textarea');
                if (textarea) {
    ...
    async function runCode(editorId) {
        ...
        if (!container || !outputDiv || !outputContent || !resultsHeader) {
            log('error', `Элементы вывода не найдены для ${editorId}`);
            return;
        }

        // Получаем код из Monaco Editor или fallback textarea
        let code = '';
        if (container._monacoEditor) {
            code = container._monacoEditor.getValue().trim();
        } else if (container.getValue) {
            code = container.getValue().trim();
        } else {
            log('error', `Не удалось получить код из редактора ${editorId}`);
            return;
        }
        if (!code) {
...
`);
    ...
    function toggleCollapse(editorId) {
        ...
        if (isCollapsed) {
            ...
            // Показываем элементы редактора кода - для editorBody используем визуальное восстановление
            if (editorBody) {
                editorBody.style.height = 'auto';
                editorBody.style.overflow = 'visible';
                editorBody.style.visibility = 'visible';
                editorBody.style.display = 'block';
            }
            // cardFooter работает правильно, оставляем как есть
            ...
            // Скрываем элементы редактора кода - для editorBody используем визуальное скрытие
            if (editorBody) {
                editorBody.style.height = '0px';
                editorBody.style.overflow = 'hidden';
                editorBody.style.visibility = 'hidden';
                // НЕ используем display: none для editorBody, чтобы Monaco Editor оставался активным
            }
            // cardFooter работает правильно, оставляем как есть
            if (cardFooter) cardFooter.style.display = 'none';
    ...
    async function pasteFromClipboard(editorId) {
        try {
            ...
            if (text) {
                ...
                if (container) {
                    if (container._monacoEditor) {
                        container._monacoEditor.setValue(text);
                    } else if (container.setValue) {
    ...
    async function copyToClipboard(editorId) {
        ...
        if (container) {
            let code = '';
            if (container._monacoEditor) {
                code = container._monacoEditor.getValue();
            } else if (container.getValue) {
                code = container.getValue();
            }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Monaco Editor будет инициализирован при первом использовании
        // Это поможет избежать конфликтов при загрузке страницы
        ...
        const observer = new MutationObserver(function(mutations) {
            ...
            mutations.forEach(function(mutation) {
                ...
                if (mutation.type === 'attributes' && mutation.attributeName === 'data-mode') {
                    ...
                    const oldMode = mutation.oldValue;
                    
                    // ВАЖНО: Срабатываем только при переходе В режим "rendered", а не ИЗ него
                    // И только если это действительно изменение (не повторная установка того же значения)
                    if (newMode === 'rendered' && oldMode !== 'rendered') {
                        // Дополнительно проверяем, есть ли в этом контейнере редакторы Python
                        const hasEditors = target.querySelector('.pyodide-editor-container');
                        if (hasEditors) {
                            log('debug', `Переинициализация редакторов после смены режима с "${oldMode}" на "rendered"`);
                            hasRelevantChanges = true;
                            shouldReinitialize = true;
                        }
                    } else {
                        log('debug', `Пропуск переинициализации для смены режима с "${oldMode}" на "${newMode}"`);
                    }
                }
                
                // Отслеживаем добавление/удаление узлов (только для важных изменений)
                if (mutation.type === 'childList') {
                    ...
                    // Проверяем, были ли удалены редакторы (только если это не массовое удаление)
                    if (mutation.removedNodes.length < 10) { // Избегаем массовых операций
            ...
            // Переинициализируем только если были действительно важные изменения
            if (shouldReinitialize && hasRelevantChanges) {
        ...
        observer.observe(document.body, {
            ...
            attributeOldValue: true, // Добавляем отслеживание старых значений атрибутов
            childList: true,
            subtree: true
        });
